...
export const getHeartAmplitude = (percent: number): number => {
  return percent === 0
    ? 0
    : Math.max(MAX_AMPLITUDE * percent, 25);
};

export const getHeartSpeed = (percent: number): number => {
...
export const getHeartSpeedByPart = (
  percent: number,
  {
    partSize,
    nextStepEnd,
